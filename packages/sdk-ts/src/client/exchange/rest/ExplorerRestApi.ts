...
import { ExplorerRestTransformer } from '../transformers'
import { Block } from '../types/explorer'

export class ExplorerRestApi extends BaseRestConsumer {
  async fetchBlock(blockHashHeight: string): Promise<BlockWithTxs> {
    try {
      ...
      return ExplorerRestTransformer.blockWithTxToBlockWithTx(
        response.data.data,
      )
    } catch (error: any) {
  ...
  async fetchBlocks(params?: { limit?: number }): Promise<Block[]> {
    try {
      ...
      return ExplorerRestTransformer.blocksToBlocks(response.data.data)
    } catch (error) {
  ...
  async fetchBlocksWithTx(params?: {
    ...
    try {
      ...
      return {
        ...
        blocks: data
          ? ExplorerRestTransformer.blocksWithTxsToBlocksWithTxs(data)
          : [],
      }
    } catch (error: any) {
      throw new HttpException(error.message)
  ...
  async fetchTransactions(params?: {
    ...
    try {
      ...
      return {
        total: paging.total > 0 ? paging.total : DEFAULT_PAGINATION_TOTAL_COUNT,
        transactions: data
          ? ExplorerRestTransformer.transactionsToTransactions(data)
          : [],
      }
    } catch (error: any) {
      throw new HttpException(error.message)
  ...
  async fetchTransaction(hash: string): Promise<Transaction> {
    try {
      ...
      return ExplorerRestTransformer.transactionToTransaction(
        response.data.data,
      )
    } catch (error: any) {
      if (error.response.status === 404) {
