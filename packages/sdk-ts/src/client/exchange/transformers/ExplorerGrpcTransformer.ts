...
export class ExplorerGrpcTransformer {
  ...
  static grpcTransactionToTransaction(tx: TxData): Transaction {
    return {
      ...
      data: tx.getData(),
      gasFee: ExplorerGrpcTransformer.grpcGasFeeToGasFee(tx.getGasFee()!),
      txType: tx.getTxType(),
  ...
  static grpcTransactionsToTransactions(
    ...
    return txs.map((tx) =>
      ExplorerGrpcTransformer.grpcTransactionToTransaction(tx),
    )
  }
  ...
  static grpcBlocksToBlocks(blocks: Array<BlockInfo>): Array<Block> {
    return blocks.map((block) =>
      ExplorerGrpcTransformer.grpcBlockToBlock(block),
    )
  }
  ...
  static grpcBlocksToBlocksWithTxs(
    ...
    return blocks.map((block) =>
      ExplorerGrpcTransformer.grpcBlockToBlockWithTxs(block),
    )
  }
  ...
  static grpcValidatorToValidator(validator: GetValidatorResponse): Validator {
    return {
      ...
      description:
        ExplorerGrpcTransformer.grpcValidatorDescriptionToValidatorDescription(
      ...
      uptimesList: validator
        .getUptimesList()
        .map(ExplorerGrpcTransformer.grpcValidatorUptimeToValidatorUptime),
      slashingEventsList: validator
        .getSlashingEventsList()
        .map(
          ExplorerGrpcTransformer.grpcValidatorSlashingEventToValidatorSlashingEvent,
        ),
    }
  }
