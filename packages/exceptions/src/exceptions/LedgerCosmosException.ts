...
export class LedgerCosmosException extends ConcreteException {
  ...
  public parse(): void {
    ...
    if (isCommonLockedError(message)) {
      this.setMessage(
        'Please ensure your Ledger is connected, unlocked and your Cosmos app is open.',
      )
    }

    if (message.includes('No device selected.')) {
      this.setMessage(
        'Please make sure your Ledger device is connected, unlocked and your Cosmos app is open',
      )
    }
