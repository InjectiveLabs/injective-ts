...
ws_price_oracle_branch=master

## Clone current proto definitions from core
git clone https://github.com/InjectiveLabs/injective-ws-price-oracle.git $BUILD_DIR -b $ws_price_oracle_branch --depth 1 --single-branch > /dev/null

# collecting indexer proto files
find $ROOT_DIR/gen/api/gen/grpc/oracle/pb -type f -name "*.proto" -exec cp {} $PROTO_DIR/ \;
